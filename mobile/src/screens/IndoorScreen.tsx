...
import type { Building, Floor, IndoorLocation } from '@/types';
...
export function IndoorScreen() {
  ...
  const [start, setStart] = useState<IndoorLocation | null>(null);
  const [end, setEnd] = useState<IndoorLocation | null>(null);
  ...
  const selectLocation = (location: IndoorLocation) => {
    if (!start) {
      setStart(location);
      return;
    }
  ...
  const runRoute = async () => {
    ...
    try {
      ...
      setPathNames([(e as Error).message]);
    } finally {
      setRouting(false);
    }
  };
      ...
      {locations.length > 0 && (
        <>
          ...
          <FlatList
            ...
            keyExtractor={(item) => String(item.id)}
            renderItem={({ item }) => (
              <Pressable style={styles.locationRow} onPress={() => selectLocation(item)}>
          ...
          />
          {start && end && (
            <Pressable style={styles.routeBtn} onPress={runRoute} disabled={routing}>
