...
function SimpleFloorPlanViewer({
  floorName = "First",
  ...
  const [imageSize, setImageSize] = useState({ width: 1e3, height: 800 });
  ...
  const handleImageLoad = () => {
    if (imageRef.current) {
      setImageSize({
        width: imageRef.current.naturalWidth || 1e3,
  ...
  >
                ...
                {userPosition && <div className="ml-auto flex items-center gap-2 bg-green-50 px-3 py-1 rounded-full border border-green-200">
  ...
  >
    ...
    /* Route Path */
                            ...
                            {route && route.waypoints && route.waypoints.length > 1 && <g>
                                    <polyline
  ...
  />
                                    {route.waypoints.map((point, idx) => <circle
    key={idx}
    cx={point.x}
    cy={point.y}
    r="6"
  ...
  />
    ...
    /* Destination Pin Visualization */
                            ...
                            {selectedEnd && <g transform={`translate(${selectedEnd.x_coordinate}, ${selectedEnd.y_coordinate})`}>
                                    {
    /* Pulsing outer ring */
  }
                                    <circle
  ...
  />
    ...
    /* Legend */
                ...
                {locations.length > 0 && <div className="absolute bottom-4 left-4 bg-white bg-opacity-95 backdrop-blur px-4 py-3 rounded-lg border border-gray-300 shadow-lg text-xs z-10">
...
export {
  SimpleFloorPlanViewer as default
};
