...
function IndoorMapViewer({
  ...
  const [nodes, setNodes] = useState(externalNodes);
      ...
      try {
        ...
        try {
          ...
          if (geoJsonResponse.ok) {
            ...
            const offices = geojson.features.map((feature, index) => ({
              id: index,
  ...
  if (loading) {
    return <div className="flex flex-col items-center justify-center h-96 bg-gray-50 rounded-lg">
  ...
  >
    ...
    return <line
      key={`path-${idx}`}
      x1={fromNode.x_coordinate}
      y1={fromNode.y_coordinate}
      x2={toNode.x_coordinate}
      y2={toNode.y_coordinate}
      stroke="#cbd5e1"
      strokeWidth="3"
      strokeLinecap="round"
      opacity="0.6"
    ...
    /* Highlighted Route - Dashed Animation */
                    ...
                    {highlightedPath && highlightedPath.length > 1 && <g id="navigation-path-layer">
    ...
    />
      ...
      cy={ap.y_coordinate}
      r={isDetected ? 10 : 6}
      fill={isDetected ? "#10b981" : "#6b7280"}
      stroke="white"
      strokeWidth="2"
    />
                                <text
      x={ap.x_coordinate}
      y={ap.y_coordinate - 15}
      textAnchor="middle"
      fill={isDetected ? "#059669" : "#4b5563"}
      fontSize="10"
      fontWeight="bold"
    >
                                    {ap.ssid}
                                </text>
                                {isDetected && <text x={ap.x_coordinate} y={ap.y_coordinate + 20} textAnchor="middle" fill="#059669" fontSize="9">
    ...
    /* User Position - Rendered from props */
                    ...
                    {userPosition && <g data-user style={{ pointerEvents: "none" }}>
                            {
    /* Accuracy radius */
  }
                            <circle
  ...
  >
    ...
    /* Label */
                    ...
                    {
    /* Destination Pin - End Location */
  }
                    {selectedEnd && <g transform={`translate(${nodes?.find((n) => n.id === selectedEnd)?.x_coordinate}, ${nodes?.find((n) => n.id === selectedEnd)?.y_coordinate})`}>
                            {
    /* Pulsing Outer Ring */
  }
                            <circle r="12" fill="#ef4444" opacity="0.75">
    ...
    />
                                ...
                                {(isStart || isEnd) && <>
                                        <rect
      x={location.x_coordinate - 28}
      y={location.y_coordinate - 28}
      width="56"
      height="20"
      rx="6"
      fill={isStart ? "#22c55e" : "#8b5cf6"}
      opacity="0.95"
    />
                                        <text
      x={location.x_coordinate}
      y={location.y_coordinate - 15}
      textAnchor="middle"
      fill="white"
      fontSize="11"
      fontWeight="bold"
                ...
                <div className="absolute bottom-4 left-4 bg-white bg-opacity-98 p-4 rounded-lg border-2 border-gray-300 text-sm shadow-lg">
                    ...
                    <div className="space-y-2">
                        <div className="flex items-center gap-3">
                            <div className="w-4 h-4 rounded-full bg-red-500 flex-shrink-0 border border-red-600" />
                            <span className="text-gray-800 font-medium">Room</span>
                        </div>
                        <div className="flex items-center gap-3">
                            <div className="w-4 h-4 rounded-full bg-green-500 flex-shrink-0 border border-green-600" />
                            <span className="text-gray-800 font-medium">Hallway</span>
                        </div>
                        <div className="flex items-center gap-3">
                            <div className="w-4 h-4 rounded-full bg-yellow-500 flex-shrink-0 border border-yellow-600" />
    ...
    /* Conditional WiFi Legend Items */
                        ...
                        {showWifiOverlay && <>
                                <div className="border-t border-gray-300 mt-2 pt-2">
                                    <div className="flex items-center gap-3">
                                        <div className="w-4 h-4 rounded-full bg-blue-500 flex-shrink-0 border-2 border-white shadow" />
                        ...
                        <div className="border-t border-gray-300 mt-2 pt-2">
                            <div className="flex items-center gap-3">
                                <div className="w-4 h-4 rounded-full bg-green-400 flex-shrink-0 border-2 border-green-500" style={{ boxShadow: "0 0 4px rgba(34, 197, 94, 0.8)" }} />
    ...
    /* Positioning Status Overlay */
                ...
                {isPositioning && <div className="absolute top-4 right-4 bg-white bg-opacity-98 p-3 rounded-lg border-2 border-gray-300 shadow-lg min-w-[150px]">
                        <div className="flex items-center gap-2 mb-2">
                            <div className="animate-spin h-4 w-4 border-2 border-blue-500 border-t-transparent rounded-full" />
                            <span className="font-medium text-sm">Scanning...</span>
                        </div>
                        
                        {userPosition ? <div className="text-xs text-blue-700 font-mono bg-blue-50 p-2 rounded">
                                <div>Acc: ±{userPosition.accuracy.toFixed(1)}m</div>
                                <div>APs: {wifiSignals.length}</div>
                            </div> : positioningError ? <div className="text-xs text-red-600 bg-red-50 p-2 rounded">{positioningError}</div> : <div className="text-xs text-amber-600">Acquiring position...</div>}
                    </div>}
            </div>
        </div>;
}
export {
  IndoorMapViewer as default
};
