...
function NavigationContainer({
  ...
  useEffect(() => {
    const loadAPs = async () => {
      try {
        ...
        console.error("Failed to load APs:", error2);
        setAccessPoints([
  ...
  const {
    ...
    scanIntervalMs: 3e3,
    onPositionUpdate: useCallback((pos) => {
      console.log("Position:", pos);
      if (pos && selectedEnd !== null) {
      }
    }, [selectedEnd])
  });
  const handleNodeClick = useCallback((nodeId, location) => {
  ...
  return <div className="flex flex-col h-screen bg-gray-100">
    ...
    className={`px-3 py-1.5 rounded text-sm font-medium transition ${positioningEnabled ? "bg-red-100 text-red-700 hover:bg-red-200" : "bg-green-100 text-green-700 hover:bg-green-200"} disabled:bg-gray-100 disabled:text-gray-400 disabled:cursor-not-allowed`}
  >
                                {positioningEnabled ? "\u23F9 Stop" : "\u25B6 Start"}
                            </button>
                            
                            {accessPoints.length < 3 && <span className="text-xs text-amber-600">Need 3+ APs</span>}
                            
                            {positioningEnabled && <button
    onClick={() => setShowWifiOverlay(!showWifiOverlay)}
    className={`px-3 py-1.5 rounded text-sm font-medium transition ${showWifiOverlay ? "bg-blue-100 text-blue-700" : "bg-gray-200 text-gray-600"}`}
  >
    ...
    /* Status Display */
                        ...
                        {positioningEnabled && <div className="flex items-center gap-3 text-sm bg-blue-50 px-3 py-2 rounded-lg border border-blue-200">
                                {userPosition ? <>
                                    ...
                                    </> : error ? <span className="text-red-600 text-xs">{error}</span> : <span className="text-amber-600 text-xs">Acquiring...</span>}
                            </div>}
                    </div>
  ...
  />
    ...
    /* Debug Panel (Optional) */
            ...
            {process.env.NODE_ENV === "development" && positioningEnabled && <div className="bg-gray-900 text-green-400 p-3 font-mono text-xs h-32 overflow-auto border-t">
                    <div className="font-bold mb-1">Debug Log:</div>
                    <div>Scanning: {isScanning ? "YES" : "NO"} | APs Configured: {accessPoints.length} | Detected: {detectedCount}</div>
                    {wifiSignals.map((s, i) => <div key={i} className="ml-2 opacity-75">
...
export {
  NavigationContainer as default
};
