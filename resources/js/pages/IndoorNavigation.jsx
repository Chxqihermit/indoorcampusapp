...
function IndoorNavigationPage() {
  const [selectedFloorIndex, setSelectedFloorIndex] = useState(2);
  ...
  const selectLocationAsStart = (location) => {
  ...
  const selectLocationAsEnd = (location) => {
  ...
  const findMyLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      { enableHighAccuracy: true, timeout: 2e4, maximumAge: 6e4 }
    );
  };
  ...
  >
    ...
    /* Search Results Dropdown */
                                    ...
                                    {showSearchResults && filteredLocations.length > 0 && <div className="absolute top-full left-0 right-0 mt-2 bg-white border-2 border-blue-300 rounded-lg shadow-2xl z-50 max-h-64 overflow-y-auto">
  ...
  >
    ...
    /* No Results */
                                    ...
                                    {showSearchResults && searchQuery && filteredLocations.length === 0 && <div className="absolute top-full left-0 right-0 mt-2 bg-white border-2 border-yellow-300 rounded-lg shadow-lg p-2 z-50 text-xs">
  ...
  >
                                    {FLOORS.map((floor, index) => <option key={index} value={index}>
                                            {floor.name}
                                        </option>)}
                                </select>
                            </div>
  ...
  >
    ...
    /* Start Location Card */
                        ...
                        {startLocation && <div className="flex-1">
  ...
  >
    ...
    /* End Location Card */
                        ...
                        {endLocation && <div className="flex-1">
                        ...
                        {!startLocation && <div className="flex-1">
                                <div className="rounded-xl border border-sidebar-border/70 bg-white p-4 dark:border-sidebar-border h-full">
                                    ...
                                    <div className="space-y-2 max-h-48 overflow-y-auto">
                                        {loadingLocations ? <div className="text-center py-4 text-gray-500">Loading locations...</div> : locations.length === 0 ? <div className="text-center py-4 text-gray-500 text-sm">No locations available</div> : locations.map((loc) => <button
    key={loc.id}
  ...
  >
                        ...
                        {startLocation && !endLocation && <div className="flex-1">
                                <div className="rounded-xl border border-sidebar-border/70 bg-white p-4 dark:border-sidebar-border h-full">
                                    <h4 className="font-semibold text-gray-900 mb-3 text-sm">Select End Location</h4>
                                    <div className="space-y-2 max-h-48 overflow-y-auto">
                                        {locations.filter((loc) => loc.id !== startLocation.id).map((loc) => <button
    key={loc.id}
  ...
  >
    ...
    /* Calculate Route Button */
                                ...
                                {startLocation && endLocation && <button
  ...
  >
    ...
    /* Route Information */
                                ...
                                {route && <div className="rounded-xl border border-blue-300 bg-blue-50 p-4">
                                        <div className="mb-3">
                                        ...
                                        <div className="space-y-1 text-xs">
                                            {route.path.map((loc, idx) => <div key={loc.id} className="text-gray-700">
                                                    {idx + 1}. {loc.name}
                                                </div>)}
                                        </div>
                                    </div>}
    ...
    /* Error Message */
                                ...
                                {routeError && <div className="rounded-xl border border-red-300 bg-red-50 p-4">
...
export {
  IndoorNavigationPage as default
};
